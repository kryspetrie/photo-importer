...
class HybridCornerDetector(
  ...
  fun detectPhotos(image: BufferedImage): List<DetectedPhoto> {
    ...
    // Step 2: Whole-image filter — reject regions covering >80% of image (the "entire desk" false
    // positive)
    val notWholeImage =
   ...
   * detection, either (a) the overlap is small, or (b) the kept detection is not much larger. This
   * correctly handles:
  ...
  private fun overlapSuppress(quads: List<DetectedQuadrilateral>): List<DetectedQuadrilateral> {
    ...
    for (candidate in sorted) {
      ...
      for (keptQuad in kept) {
        ...
        // If the candidate overlaps significantly (>40%) with a much larger (>40% bigger) kept
        // detection,
