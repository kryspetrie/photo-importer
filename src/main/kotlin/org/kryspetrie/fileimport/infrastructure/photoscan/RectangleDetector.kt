...
class RectangleDetector(
  ...
  fun detectRectangles(
    ...
    return suppressed.map { quad ->
      quad.copy(
          corners = quad.corners.map { Point((it.x * scale).toInt(), (it.y * scale).toInt()) },
          area = (quad.area * scale * scale).toInt(),
          centroid = Point((quad.centroid.x * scale).toInt(), (quad.centroid.y * scale).toInt()))
    }
  }

  /**
   * Returns the input image unchanged if both dimensions are ≤ [maxImageDimension], otherwise
   * returns a resized copy scaled down to fit within that bound. The returned [Float] is the scale
   * factor (original / downsampled) — divide coordinates by this to go from original → downsampled,
   * multiply to go from downsampled → original.
   */
  private fun maybeDownsample(image: BufferedImage): Pair<BufferedImage, Float> {
    ...
    g.drawImage(image.getScaledInstance(newW, newH, BufferedImage.SCALE_AREA_AVERAGING), 0, 0, null)
    g.dispose()
  ...
  private fun traceContour(
    ...
    var x = startX
    var y = startY
    var dir = 0 // 0=right, 1=down-right, 2=down, 3=down-left, 4=left, 5=up-left, 6=up, 7=up-right
  ...
  private fun perpendicularDistance(point: Point, lineStart: Point, lineEnd: Point): Double {
    ...
    val len = hypot(dx, dy)
    if (len < 1e-9) return hypot(point.x - lineStart.x.toDouble(), point.y - lineStart.y.toDouble())
    return abs((dy * point.x - dx * point.y + lineEnd.x * lineStart.y - lineEnd.y * lineStart.x)) /
        len
  }
