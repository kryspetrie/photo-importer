...
class RotationTransformer {
  ...
  fun detectRotationAngle(corners: BoundingBoxCorners): Double {
    // Calculate angles of the four edges
    val topEdge = angleOfEdge(corners.topLeft, corners.topRight)
    val rightEdge = angleOfEdge(corners.topRight, corners.bottomRight)
    val bottomEdge = angleOfEdge(corners.bottomRight, corners.bottomLeft)
    val leftEdge = angleOfEdge(corners.bottomLeft, corners.topLeft)

    // Average horizontal (top and bottom) and vertical (left and right)
    val horizontalAvg = normalizeAngle((topEdge + bottomEdge) / 2)
    val verticalAvg = normalizeAngle((leftEdge + rightEdge) / 2)

    // The rotation needed is to bring horizontal edges to 0 degrees
    return horizontalAvg
  }
