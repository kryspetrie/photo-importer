 ...
 * 4. **Other**: CPU only
 *
 * When a GPU EP fails to initialize (missing driver, no hardware, library mismatch),
 * the factory silently falls back to CPU so the application always works.
 *
 * ## Session options lifecycle
 *
 * Each attempt to enable a GPU provider creates a fresh [SessionOptions] — if the provider
 * fails, the tainted options are discarded. This avoids the problem where a failed
 * `addCoreML()`/`addCUDA()`/`addDirectML()` call leaves options in an inconsistent state
 * (ONNX Runtime doesn't support removing a provider from options).
 *
 * ## Single responsibility
 *
 * Every ONNX session in the app should be created through this factory. This ensures
 * consistent GPU-usage behavior across all models (detection, pose, corner regression,
 * face detection) and centralizes session configuration (thread count, optimization level).
 *
...
class OrtSessionFactory(
     ...
     *
     * Set lazily on first session creation and then cached. Accessing before any session is
     * created returns [Provider.NONE].
     */
     ...
     *
     * The model bytes are passed directly to ONNX Runtime — no temporary files. Session
     * options are configured with:
    ...
    fun createSession(modelBytes: ByteArray): OrtSession {
        ...
        return if (isGpuAvailable) {
            tryCreateWithProvider(modelBytes, activeProvider, intraThreads)
                ...
                .also { activeProvider = Provider.CPU }
        } else {
            createCpuSessionInternal(modelBytes, intraThreads)
        }
    }
    ...
    private fun providerPriority(): List<Provider> = when {
     ...
     *
     * Returns null if the provider is unavailable (throws [OrtException]).
     * Creates fresh [SessionOptions] for each attempt — if the `addXxx()` call throws,
     * the options are discarded cleanly.
     */
    ...
    private fun createCpuSessionInternal(
        modelBytes: ByteArray,
        intraThreads: Int,
    ): OrtSession {
        val opts = SessionOptions()
