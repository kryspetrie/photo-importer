...
class NamingAdapter : NamingPort {
  override fun generateFilePath(
    ...
    return "${generateFolderPath(imageFile, destinationRoot, configuration)}/${generateFileName(imageFile, configuration, counter)}"
  }
  ...
  override fun generateFileName(
    ...
    var fileNamePattern = configuration.fileNamePattern.replace("{original}", fileNameWithoutExt)
    ...
    fileNamePattern =
        replaceDateTokens(fileNamePattern, date)
            .replace("{camera}", sanitizeFileName(meta?.cameraModel ?: ""))
            .replace("{make}", sanitizeFileName(meta?.cameraMake ?: ""))
            .replace("{lens}", sanitizeFileName(meta?.lensModel ?: ""))
            .replace("{iso}", meta?.iso?.toString() ?: "")
            .replace("{aperture}", meta?.aperture?.let { "f${it}" } ?: "")
            .replace("{shutter}", sanitizeFileName(meta?.shutterSpeed ?: ""))
            .replace("{focal}", meta?.focalLength?.let { "${it.toInt()}mm" } ?: "")
            .replace("{focal35}", meta?.focalLength35mm?.let { "${it}mm" } ?: "")
            .replace("{width}", meta?.imageWidth?.toString() ?: "")
            .replace("{height}", meta?.imageHeight?.toString() ?: "")
            .replace("{counter}", counter.toString().padStart(4, '0'))
            .replace("{type}", mediaType)
            .replace("{duration}", durationStr)
            .replace("{fps}", meta?.frameRate?.let { "%.0f".format(it) } ?: "")
            .replace("{codec}", sanitizeFileName(meta?.videoCodec ?: ""))
