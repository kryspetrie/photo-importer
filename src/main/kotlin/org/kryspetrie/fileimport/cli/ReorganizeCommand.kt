...
class ReorganizeCommand(private val reorganizeService: ReorganizeService) :
  ...
  private val mode by
      option("--mode", help = "Operation mode: move or copy").choice("move", "copy").default("move")
  ...
  private val preserveOriginalName by
      option("--preserve-original-name", help = "Preserve original filename").flag(default = false)
  ...
  override fun run() = runBlocking {
    ...
    if (dryRun) {
      echo("Scanning files...")
      val preview =
          reorganizeService.scanAndPreview(source, config, renameOnly, operationMode) { progress ->
            if (progress.total > 0) {
      ...
      if (preview.changedFiles > 0) {
        ...
        preview.mappings
            ...
            .take(10)
            .forEach { mapping ->
              echo("  ${mapping.currentPath} -> ${mapping.newPath} (${mapping.file.fileName})")
            }
  ...
  override fun run() = runBlocking {
    ...
    val result =
        reorganizeService.undo(journalPath) { progress ->
          echo("${progress.phase}: ${progress.current}/${progress.total} - ${progress.currentFile}")
        }
...
class CheckReorganizeJournalsCommand(private val reorganizeService: ReorganizeService) :
    CliktCommand(
        name = "check-journals", help = "List available reorganization journals for undo") {
  override fun run() = runBlocking {
    echo("=".repeat(50))
