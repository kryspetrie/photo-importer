 ...
 *
 * Shows download progress, errors, and retry options. The dialog is shown when the user
 * tries to use auto-rotate but the model is not available locally.
 *
...
fun ModelDownloadDialog(
    ...
    AlertDialog(
        onDismissRequest = {
            if (downloadState !is ModelDownloadState.Downloading) onCancel()
        },
        title = { Text("Download Orientation Model") },
        text = {
            Column(verticalArrangement = Arrangement.spacedBy(12.dp)) {
                when (downloadState) {
                    null, is ModelDownloadState.Idle -> {
                        Text(
                    ...
                    is ModelDownloadState.Connecting -> {
                        Text("Connecting to download server...", style = MaterialTheme.typography.bodyMedium)
                    ...
                    is ModelDownloadState.Downloading -> {
                        ...
                        if (downloadState.progressPercent != null) {
                            ...
                            LinearProgressIndicator(modifier = Modifier.fillMaxWidth())
                            Text(
                                "%.1f MB downloaded".format(bytesToMb(downloadState.bytesDownloaded)),
                                style = MaterialTheme.typography.labelSmall,
                                color = MaterialTheme.colorScheme.onSurfaceVariant,
                            )
                        }
        ...
        confirmButton = {
            when (downloadState) {
                ...
                is ModelDownloadState.Connecting, is ModelDownloadState.Downloading -> {
