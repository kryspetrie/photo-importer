...
fun WizardContainer(
    ...
    Scaffold(
        topBar = {
            ...
            val isImportOrProcessing = currentStep == PhotoScanWizardState.WizardStep.IMPORT ||
                currentStep == PhotoScanWizardState.WizardStep.PROCESSING
            val isCompleteOrRefinement = currentStep == PhotoScanWizardState.WizardStep.COMPLETE ||
                currentStep == PhotoScanWizardState.WizardStep.REFINEMENT
            if (!isImportOrProcessing && !isCompleteOrRefinement) {
                TopAppBar(
                    ...
                    Snackbar(
                        ...
                        action = {
                            TextButton(
                                onClick = {
                                    errorMessage = null
                                    // If we have an image, go back to overview; otherwise back to
                                    // import
        ...
        boxes.forEachIndexed { index, box ->
            ...
            appLogger.logOperationStart(
                OperationType.EXPORT_PHOTO,
                    "Photo ${index + 1}/${boxes.size}: $fileName " +
            ...
            val detectedPhoto =
                DetectedPhoto(
                    topLeft =
                        PhotoCorner(
                            box.corners.topLeft.x.toFloat(),
                            box.corners.topLeft.y.toFloat(),
                        ),
                    topRight =
                        PhotoCorner(
                            box.corners.topRight.x.toFloat(),
                            box.corners.topRight.y.toFloat(),
                        ),
                    bottomLeft =
                        PhotoCorner(
                            box.corners.bottomLeft.x.toFloat(),
                            box.corners.bottomLeft.y.toFloat(),
                        ),
                    bottomRight =
                        PhotoCorner(
