 ...
 *
 * Defines a cohesive set of colors following Material Design 3 guidelines
 * optimized for desktop applications. The color scheme uses a blue primary
 * color with complementary secondary and tertiary colors for a professional,
 ...
 *
 * Defines colors optimized for dark mode, reducing eye strain in low-light
 * environments while maintaining brand identity and accessibility. Dark theme
 * uses lighter versions of the brand colors to ensure sufficient contrast
 ...
 *
 * Defines text styles for all text elements in the application, from large
 * headlines to small labels. Desktop typography uses larger sizes and different
 * line heights compared to mobile to account for viewing distance and screen size.
 *
 * ## Typography Scale
 *
 ...
 *
 * Defines corner radius values for all rounded elements in the application,
 * from buttons to cards to dialogs. Uses a consistent scale based on
 ...
 * ## Recomposition
 *
 * When the theme changes, all composables within this theme will recompose
 * with the new colors, typography, and shapes. This is automatic - no manual
 * refresh needed.
 *
 ...
 *
 * @param appTheme Theme mode selection (light/dark/system).
 *                 Changes to this parameter trigger theme recomposition.
 * @param content The UI content to which the theme applies.
 *                Should contain the entire application UI tree.
 *
 * @see AppTheme Theme selection enum
...
fun PetrieTheme(
     ...
     * UI content to theme.
     *
     * This lambda contains all UI composables that should receive the theme.
     * Typically includes the entire application UI tree.
     *
     * Example:
     * ```kotlin
     * PetrieTheme {
