 ...
 * values to the current photo — useful when importing a series of photos taken at the same time and
 * location.
 *
 * Each set is timestamped so the most-recently-used entries appear first. Fields that were not set
 * (blank in the original config) remain blank here, so applying a set only overwrites fields the
 * user originally filled in (matching the multi-edit "apply non-blank" convention).
 *
 * Note: Face selection (face regions) is excluded from recent sets, since those are photo-specific
 * coordinates rather than reusable metadata text values.
 */
@Serializable
data class RecentMetadataSet(
    ...
    /**
     * A short human-readable label for this set, derived from the most identifying fields. Prefers
     * location-based labels (e.g. "Grandma's house, Worcester, MA"), falls back to description,
     * date, or "unlabeled set".
     */
    val label: String
        get() {
            val locationLabel =
                listOf(locationName, city, state, country)
    ...
    /**
     * Returns true if any field in this set is non-blank. A completely empty set is not useful to
     * offer in the UI.
     */
    ...
    /**
     * Returns a summary line showing 2–3 key fields for compact display. E.g. "Worcester, MA ·
     * 2024-06-15 · Canon"
     */
    val summary: String
        get() {
            ...
            if (originalDate.isNotBlank()) parts.add(originalDate)
            else if (year.isNotBlank()) parts.add(year)
    ...
    /**
     * Applies this set's non-blank fields over an existing [PhotoScanConfiguration], returning a
     * new config with the merged values.
     *
     * Blank fields in this set are skipped, preserving the original config's values. This matches
     * the multi-edit "apply non-blank" convention.
     */
    fun mergeInto(config: PhotoScanConfiguration): PhotoScanConfiguration =
        config.copy(
     ...
     */
    fun mergeLocationInto(config: PhotoScanConfiguration): PhotoScanConfiguration =
        config.copy(
            locationName = if (locationName.isNotBlank()) locationName else config.locationName,
            address = if (address.isNotBlank()) address else config.address,
            city = if (city.isNotBlank()) city else config.city,
