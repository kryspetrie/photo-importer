 ...
 *
 * In production, these delegate to [DefaultIdGenerator] and [DefaultTimeProvider].
 * Tests can override via [setIdGenerator] and [setTimeProvider] to control values.
 *
 * This replaces direct `java.util.UUID.randomUUID()` and `System.currentTimeMillis()`
 * calls in domain model default parameters, making domain models testable without
 * hardcoding JVM dependencies.
 */
object DomainDefaults {
    ...
    @Volatile
    private var timeProvider: TimeProvider = DefaultTimeProvider()
    ...
    /**
     * Format a timestamp as a human-readable string using the current [TimeProvider].
     * Format: "yyyy-MM-dd HH:mm:ss"
     */
    fun formatTimestamp(timestamp: Long): String = timeProvider.formatTimestamp(timestamp)

    /**
     * Override the [IdGenerator] for testing.
     * Returns a reset function to restore the default.
     */
    ...
    /**
     * Override the [TimeProvider] for testing.
     * Returns a reset function to restore the default.
     */
