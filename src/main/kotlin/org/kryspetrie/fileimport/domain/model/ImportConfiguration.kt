 ...
 *
 * This data class encapsulates all settings that control how files are imported,
 * organized, named, and processed. It's the core configuration object that drives
 * the entire import workflow.
 *
 ...
 * ## Serialization
 *
 * Marked with `@Serializable` for JSON persistence in import profiles.
 * All settings are preserved when saving/loading profiles.
 *
 * @property folderPattern Pattern for creating subfolders. Uses placeholders
 *                        like `{yyyy-MM-dd}`, `{camera}`, etc. See [FolderPresets]
 *                        for built-in patterns and examples.
 * @property fileNamePattern Pattern for generating filenames. Used when
 *                           [preserveOriginalName] is false. See [FilenamePresets]
 *                           for built-in patterns.
 * @property fileNameExtension File extension pattern. Usually `{ext}` to preserve
 *                             original extension.
 * @property preserveOriginalName If true, keep original filename. If false,
 *                                generate new name using [fileNamePattern].
 * @property createSubfolders If true, create folder hierarchy using [folderPattern].
 *                            If false, put all files in destination root (flat).
 * @property dateSource Which date to use for date-based patterns. Options:
 *                      EXIF date (when taken), file modified, or file created.
 * @property conflictResolution Strategy for handling filename conflicts:
 *                              Rename (add number), Skip, Replace, or Ask User.
 * @property detectTransferredByHash Detect already-imported files by comparing
 *                                   file hashes. Prevents importing same file twice.
 * @property detectTransferredByExif Detect duplicates by EXIF metadata (date, camera,
 *                                   dimensions). Catches edited versions of same photo.
 * @property detectVisualDuplicates Use perceptual hashing to find visually similar
 *                                  images. Catches resizes, crops, slight edits.
 * @property perceptualHashThreshold Similarity threshold for perceptual hash matching.
 *                                  1.0 = exact match, 0.95 = very similar, 0.8 = similar.
 * @property useSurfMatching Enable SURF feature matching for advanced duplicate
 *                          detection. Slower but more accurate than perceptual hash.
 * @property surfMatchThreshold Number of matching features required for SURF match.
 *                              Higher = more strict (fewer false positives).
 * @property rawJpegPairMode How to handle RAW+JPEG pairs: Import both, RAW only,
 *                           or JPEG only.
 * @property keepPairsTogether If true, keep RAW+JPEG pairs in same folder even if
 *                             pattern would separate them.
 * @property importSidecars If true, import sidecar files (.xmp, .thm, .lrv, etc.)
 *                          alongside media files.
 * @property verifyAfterCopy If true, calculate hash after copy and compare to source.
 *                           Ensures file integrity. Recommended for safety.
 * @property deleteAfterImport If true, delete source files after successful import
 *                             and verification. Use with caution!
 *
...
data class ImportConfiguration(
     ...
     *
     * Usually `{ext}` to preserve original extension (.jpg, .cr3, .mp4, etc.).
     * Can be customized for specific needs (e.g., force .jpeg instead of .jpg).
     *
     * Default: `{ext}`
     */
     ...
     *
     * Compares file hash against import history database.
     * Catches exact duplicates (same file imported before).
     *
     ...
     *
     * SURF (Speeded Up Robust Features) detects and matches visual features
     * between images. More accurate than perceptual hash but significantly
     * slower.
     *
     * Best for:
     ...
     * Lower threshold = more lenient (catches more duplicates)
     *
     * Typical range: 20-50
     *
     * Default: `30`
     ...
     *
     * Many cameras save both RAW and JPEG for each shot. This setting
     ...
     *
     * Professional photographers often import both for flexibility.
     * Casual users may prefer JPEG only to save space.
     *
     * Default: `IMPORT_BOTH`
     */
     ...
     *
     * If true, ensures RAW and JPEG from same shot stay in same folder
     * even if folder pattern would normally separate them.
     *
     ...
     * Highly recommended for important photos.
     *
 ...
 *
 * Separate from [ImportConfiguration] to allow fine-tuning duplicate
 * detection independently from import settings. Used primarily in
 * the Duplicate Scanner screen.
 *
 ...
 *
 * Determines which timestamp is used when resolving date placeholders
 * like `{yyyy}`, `{MM}`, `{dd}`, `{HH}`, etc.
 *
 * ## Options
 *
 ...
 * ## Recommendation
 *
 * Use **EXIF_DATE** for photos (most accurate).
 * For videos without EXIF, system falls back to file dates automatically.
 *
 ...
 *
 * Determines what happens when a file with the same name already
 * exists at the import destination.
 *
 * ## Options
 *
 ...
 * ## Recommendation
 *
 * Use **RENAME** for safety and automation.
 * Use **SKIP** if you want to manually review conflicts later.
 * Avoid **REPLACE** unless you have backups.
 *
 ...
 * ## Usage
 *
 * Selected in Import screen via radio buttons or dropdown.
 * Affects which files are shown in preview and imported.
 *
 ...
 *
 * Many cameras (especially DSLRs and mirrorless) can save both RAW and JPEG
 * versions of each photo. This enum determines which files to import.
 *
 * ## Options
 *
 ...
 *
 * Presets allow users to save and reuse custom patterns.
 * Built-in presets are provided by [FolderPresets] and [FilenamePresets],
 * and users can create their own custom presets.
 *
 * ## Usage
 *
 * ```kotlin
 ...
 * Users can select these from a dropdown instead of typing patterns manually.
 *
 * ## Included Patterns
 *
...
object FolderPresets {
   ...
   *
   * Map of pattern → example output path.
   * Useful for showing users what the pattern produces.
   *
   * Example:
   * ```
 ...
 * Users select these instead of typing patterns manually.
 *
 * ## Included Patterns
 *
 ...
 *
 * Placeholders are replaced with actual values during import.
 * Different placeholders are available for folder patterns vs. filename patterns.
 *
 * ## Folder Placeholders
 *
...
object NamePlaceholders {
   ...
   *
   * Map of placeholder → description.
   * Used for help text and validation.
   */
   ...
   *
   * Map of placeholder → description.
   * More extensive than folder placeholders, includes EXIF data.
   */
