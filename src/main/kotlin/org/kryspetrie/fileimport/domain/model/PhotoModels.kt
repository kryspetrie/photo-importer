...
data class PercentPoint(val x: Double, val y: Double) {
  ...
  fun toPixels(imageWidth: Int, imageHeight: Int): PixelPoint {
    return PixelPoint(
        x = (x / 100.0 * imageWidth).toFloat(), y = (y / 100.0 * imageHeight).toFloat())
  }
}
...
data class PercentBoundingBox(
  ...
  fun moveCorner(corner: CornerType, newPosition: PercentPoint): PercentBoundingBox {
    val clampedPosition =
        PercentPoint(x = newPosition.x.coerceIn(0.0, 100.0), y = newPosition.y.coerceIn(0.0, 100.0))
...
enum class AspectRatio(val width: Int?, val height: Int?) {
  ...
  val ratio: Double?
    get() = if (width != null && height != null && height != 0) width.toDouble() / height else null
