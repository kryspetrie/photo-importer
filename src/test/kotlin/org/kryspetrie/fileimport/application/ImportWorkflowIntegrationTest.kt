...
class ImportWorkflowIntegrationTest {
  ...
  fun setup() {
    ...
    importService = ImportService(imageRepository, deduplicationPort, namingPort)
  }
  ...
  inner class FullWorkflowScenarios {
    ...
    fun shouldProcessSinglePhotoImport() = runTest {
      ...
      val config = ImportConfiguration(folderPattern = "{yyyy-MM-dd}", preserveOriginalName = true)
  ...
  inner class PairDetectionScenarios {
    ...
    fun shouldDetectMultiplePairs() = runTest {
      ...
      val photos =
          (1..5).flatMap { i ->
            listOf(
                createTestFile("IMG_${String.format("%04d", i)}.cr2", ImageFileType.RAW_CR2),
                createTestFile("IMG_${String.format("%04d", i)}.jpg"))
          }
  ...
  inner class FilterScenarios {
    ...
    fun shouldHandleRawOnlyMode() = runTest {
      ...
      val photos =
          listOf(
              createTestFile("IMG_0001.cr2", ImageFileType.RAW_CR2), createTestFile("IMG_0001.jpg"))
  ...
  inner class LargeDatasetScenarios {
    ...
    fun shouldHandleLargeNumberOfPhotos() = runTest {
      ...
      val photos = (1..100).map { i -> createTestFile("IMG_${String.format("%04d", i)}.jpg") }
      val config = ImportConfiguration()
