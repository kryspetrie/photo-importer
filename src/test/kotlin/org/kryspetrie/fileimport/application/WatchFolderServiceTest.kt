...
class WatchFolderServiceTest {
  ...
  fun setup() {
    ...
    service = WatchFolderService(importService)
  }
  ...
  fun shouldStopWatching() = runBlocking {
    ...
    service.startWatching(
        WatchFolderConfig(
            watchPath = watchDir.absolutePath, destinationPath = tempDir.absolutePath),
        scope)
  ...
  fun shouldReportErrorForMissingPath() = runBlocking {
    ...
    service.startWatching(
        WatchFolderConfig(
            watchPath = "/nonexistent/path/watch", destinationPath = tempDir.absolutePath),
        scope)
