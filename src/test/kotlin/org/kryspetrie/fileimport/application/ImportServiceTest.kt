...
class ImportServiceTest {
  ...
  fun setup() {
    ...
    service = ImportService(imageRepository, deduplicationPort, namingPort)
  }
  ...
  inner class TransferredFiltering {
    ...
    fun shouldReturnAllWhenDisabled() {
      ...
      val destHashes = setOf("hash_a")
      val config =
          ImportConfiguration(detectTransferredByHash = false, detectTransferredByExif = false)
  ...
  inner class ImportExecution {
    ...
    fun shouldSkipOnConflict() = runTest {
      ...
      val image = ImageFile(file = sourceFile)
      val config =
          ImportConfiguration(conflictResolution = ConflictResolution.SKIP, verifyAfterCopy = false)
  ...
  inner class ScanSource {
    ...
    fun shouldThrowForMissingDir() = runTest {
      assertThrows<IllegalArgumentException> { service.scanSource("/nonexistent/path") }
    }
  }
}
