...
class MetadataEditServiceTest {
    ...
    fun setUp() {
        runBlocking {
            whenever(undoService.createBackup(any())).thenReturn("/tmp/backup.jpg")
        }
        service =
            MetadataEditService(
    ...
    inner class SaveFileRotationRoutingTests {
        @Test
        fun jpegRotationPhysicallyRotatesPixelsAndResetsOrientation() =
            runBlocking {
        ...
        @Test
        fun rawRotationUsesMetadataOnlyPathWithoutDecodingImage() =
            runBlocking {
        ...
        @Test
        fun metadataOnlyEditsSkipImageDecodingForJpeg() =
            runBlocking {
                val jpeg = createJpegFile()
                whenever(imageRepository.getMetadata(any()))
    ...
    private class TestProcessedImage(override val width: Int, override val height: Int) : ProcessedImage
}
