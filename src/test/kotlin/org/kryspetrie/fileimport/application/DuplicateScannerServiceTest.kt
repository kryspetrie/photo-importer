...
class DuplicateScannerServiceTest {
  ...
  fun setup() {
    ...
    service = DuplicateScannerService(imageRepository, deduplicationPort, null)
  }
  ...
  inner class ScanForDuplicates {
    ...
    fun shouldReturnEmptyForSingleFile() = runTest {
      ...
      f.writeText("data")
      whenever(imageRepository.scanDirectory(any(), any())).thenReturn(listOf(ImageFile(file = f)))
  ...
  inner class ResolveGroup {
    ...
    fun shouldKeepHighestRes() = runTest {
      ...
      f2.writeText("low")
      val img1 =
          ImageFile(file = f1, metadata = ImageMetadata(imageWidth = 4000, imageHeight = 3000))
      val img2 =
          ImageFile(file = f2, metadata = ImageMetadata(imageWidth = 1920, imageHeight = 1080))
    ...
    fun shouldDeleteWithoutTrash() = runTest {
      ...
      val img1 =
          ImageFile(file = f1, metadata = ImageMetadata(imageWidth = 4000, imageHeight = 3000))
      val img2 = ImageFile(file = f2, metadata = ImageMetadata(imageWidth = 800, imageHeight = 600))
      val group = DuplicateInfo(img1, listOf(img2), DuplicateType.EXACT_HASH)
  ...
  inner class ResolveAll {
    ...
    fun shouldResolveAllGroups() = runTest {
      ...
      val img2 = ImageFile(file = f2, metadata = ImageMetadata(imageWidth = 800, imageHeight = 600))
      val img3 =
          ImageFile(file = f3, metadata = ImageMetadata(imageWidth = 5000, imageHeight = 4000))
      val img4 = ImageFile(file = f4, metadata = ImageMetadata(imageWidth = 640, imageHeight = 480))
