...
class PhotoScanConfigurationTest {
    ...
    inner class Copy {
        ...
        fun `copy should preserve all fields`() {
            val original = PhotoScanConfiguration(
...
class PhotoScanProfileConfigurationTest {
    ...
    inner class NamingPatterns {
        ...
        fun `naming pattern can include counter`() {
            val profile = PhotoScanProfile(
                name = "Counter Test",
                namingPattern = "{original}_{counter:3}"
            )
    ...
    inner class OutputFormats {
        ...
        fun `JPEG formats have increasing quality`() {
            val jpegFormats = PhotoOutputFormat.entries.filter { 
                it.name.contains("JPEG") 
            }.sortedBy { it.quality }

            assertThat(jpegFormats).hasSizeGreaterThan(1)
            for (i in 0 until jpegFormats.size - 1) {
                assertThat(jpegFormats[i].quality)
                    .isLessThan(jpegFormats[i + 1].quality)
            }
        }
        ...
        fun `lossless formats have quality 100`() {
            ...
            losslessFormats.forEach { format ->
                assertThat(format.quality).isEqualTo(100)
            }
        }
    }
