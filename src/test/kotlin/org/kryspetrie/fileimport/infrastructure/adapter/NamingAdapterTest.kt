...
class NamingAdapterTest {
  ...
  inner class GenerateFileName {
    ...
    fun shouldPreserveOriginalName() {
      // GIVEN
      val config = ImportConfiguration(fileNamePattern = "{original}", preserveOriginalName = true)
    ...
    fun shouldUseDatePattern() {
      ...
      val config =
          ImportConfiguration(
              fileNamePattern = "{yyyy}-{MM}-{dd}_{original}", preserveOriginalName = false)
    ...
    fun shouldUseCounterPattern() {
      // GIVEN
      val config =
          ImportConfiguration(fileNamePattern = "Photo_{counter}", preserveOriginalName = false)
    ...
    fun shouldIncludeExifMetadata() {
      ...
      val config =
          ImportConfiguration(
              fileNamePattern = "{original}_{iso}_{aperture}", preserveOriginalName = false)
  ...
  inner class VideoPlaceholders {
    ...
    fun shouldResolveDuration() {
      ...
      val config =
          ImportConfiguration(
              fileNamePattern = "{original}_{duration}", preserveOriginalName = false)
    ...
    fun shouldResolveFpsAndCodec() {
      ...
      val config =
          ImportConfiguration(
              fileNamePattern = "{original}_{fps}fps_{codec}", preserveOriginalName = false)
      val name = adapter.generateFileName(videoFile, config, 1)
  ...
  inner class PreviewFileStructure {
    ...
    fun shouldGeneratePreview() {
      ...
      val images =
          listOf(
              testImageFile, testImageFile.copy(id = "test-2", file = File("/source/IMG_1235.jpg")))
