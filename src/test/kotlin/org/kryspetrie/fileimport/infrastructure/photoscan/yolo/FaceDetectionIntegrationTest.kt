 ...
 *
 * Reference coordinates were captured from [FaceDetectionGoldTest] running the YOLO12n face
 * detection model with confThreshold=0.50, iouThreshold=0.45.
 *
 * Tolerance is ±30px for bounding-box coordinates (out of ~2688px width) and ±0.05 for confidence
 * scores, accounting for floating-point non-determinism across different hardware and ONNX Runtime
 * backends.
 *
    ...
    inner class Faces01Test {
        ...
        fun allBoundingBoxesHavePositiveArea() {
            ...
            for ((i, det) in results.withIndex()) {
                assertThat(det.x2 - det.x1).`as`("Face $i width").isGreaterThan(0f)
                assertThat(det.y2 - det.y1).`as`("Face $i height").isGreaterThan(0f)
            }
        }
