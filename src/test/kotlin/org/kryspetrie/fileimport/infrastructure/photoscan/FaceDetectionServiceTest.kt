 ...
 *
 * Uses Mockito to stub [ModelResourcePort] and [YoloFaceDetectionService]. Reflection injects a
 * mock [YoloFaceDetectionService] into the lazy `faceService` delegate to avoid needing a real ONNX
 * Runtime session, while still exercising the actual [FaceDetectionService.detectFaces] mapping
 * logic.
 */
class FaceDetectionServiceTest {
    ...
    inner class DetectFacesTests {
        ...
        fun throwsWhenModelNotAvailable() {
            whenever(modelResourcePort.isFaceDetectionModelAvailable()).thenReturn(false)
            val service = FaceDetectionService(modelResourcePort, OrtSessionFactory())
            val image = AwtProcessedImage(BufferedImage(100, 100, BufferedImage.TYPE_INT_RGB))
        ...
        @Test
        @DisplayName(
            "converts ProcessedImage to BufferedImage and delegates to YoloFaceDetectionService"
        )
        fun convertsAndDelegatesToYoloService() {
            ...
            whenever(yoloService.detectFaces(bufferedImage, 0.5f, 0.45f)).thenReturn(emptyList())
        ...
        fun mapsDetectedFaceCorrectly() {
            ...
            val face1 =
                YoloFaceDetectionService.FaceDetection(
                    x1 = 10.5f,
                    y1 = 20.5f,
                    x2 = 110.5f,
                    y2 = 120.5f,
            ...
            assertThat(results[1])
                .isEqualTo(
                    DetectedFace(x1 = 200f, y1 = 300f, x2 = 400f, y2 = 500f, confidence = 0.8f)
                )
    ...
    /**
     * Injects a mock [YoloFaceDetectionService] into the `faceService` lazy delegate of
     * [FaceDetectionService] via reflection.
     *
    ...
    private fun FaceDetectionService.injectYoloService(mock: YoloFaceDetectionService) {
        val delegateField =
            FaceDetectionService::class.java.getDeclaredField("faceService\$delegate")
