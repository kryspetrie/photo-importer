...
class RectangleDetectorTest {
    ...
    fun `detected corners are within image bounds`() {
        ...
        for (quad in results) {
            for (corner in quad.corners) {
                assertTrue(
                    corner.x >= 0 && corner.x <= image.width,
                    "Corner x=${corner.x} out of image bounds",
                )
                assertTrue(
                    corner.y >= 0 && corner.y <= image.height,
                    "Corner y=${corner.y} out of image bounds",
                )
            }
        }
    }
    ...
    fun `handles empty image gracefully`() {
        val image =
            java.awt.image.BufferedImage(100, 100, java.awt.image.BufferedImage.TYPE_BYTE_GRAY)
    ...
    private fun cornersMatch(
        ...
        for (i in 0 until 4) {
            if (
                abs(detSorted[i].x - truthSorted[i].x) > tolerance ||
