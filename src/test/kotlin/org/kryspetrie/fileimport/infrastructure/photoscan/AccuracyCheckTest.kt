...
class AccuracyCheckTest {
  ...
  private fun matchQuadsToGroundTruth(
    ...
    for ((qIdx, quad) in quads.withIndex()) {
      var bestGTIdx = -1
  ...
  fun `detect rectangles with contour method`() {
    ...
    for ((i, gt) in groundTruth.withIndex()) {
      ...
      val area =
          kotlin.math.abs(
              gt.mapIndexed { idx, p -> p.x * gt[(idx + 1) % 4].y - gt[(idx + 1) % 4].x * p.y }
                  .sum() / 2)
    ...
    for ((idx, quad) in results.withIndex()) {
      ...
      println("  Quad[$idx]: centroid=(${quad.centroid.x}, ${quad.centroid.y}), area=${quad.area}")
