...
class DebugGT1MissedTest {
  ...
  fun `debug GT1 detection - show all detected quads with centroids`() {
    ...
    val gt0 = listOf(
        PhotoCorner(270f, 358f), PhotoCorner(1864f, 358f),
        PhotoCorner(1864f, 1452f), PhotoCorner(270f, 1452f))
    val gt1 = listOf(
        PhotoCorner(256f, 1560f), PhotoCorner(2104f, 1560f),
        PhotoCorner(2104f, 3814f), PhotoCorner(256f, 3814f))
    val gt2 = listOf(
        PhotoCorner(2226f, 634f), PhotoCorner(3700f, 634f),
        PhotoCorner(3700f, 2510f), PhotoCorner(2226f, 2510f))

    val gtCentroids = listOf(
        "GT0" to ((gt0[0].x + gt0[1].x + gt0[2].x + gt0[3].x) / 4 to 
                  (gt0[0].y + gt0[1].y + gt0[2].y + gt0[3].y) / 4),
        "GT1" to ((gt1[0].x + gt1[1].x + gt1[2].x + gt1[3].x) / 4 to 
                  (gt1[0].y + gt1[1].y + gt1[2].y + gt1[3].y) / 4),
        "GT2" to ((gt2[0].x + gt2[1].x + gt2[2].x + gt2[3].x) / 4 to 
    ...
    for ((i, quad) in quads.withIndex()) {
      ...
      println("  Quad[$i]: centroid=(${cx.toInt()}, ${cy.toInt()}), area=${quad.area}, " +
      ...
      if (gt != null) {
        val sortedCorners = sortCorners(corners.map { PhotoCorner(it.x.toFloat(), it.y.toFloat()) })
        val errors = sortedCorners.mapIndexed { idx, det ->
          hypot((det.x - gt[idx].x).toDouble(), (det.y - gt[idx].y).toDouble())
        }
        println("    Errors: TL=${"%.1f".format(errors[0])}px, TR=${"%.1f".format(errors[1])}px, " +
    ...
    println("\n=== Summary ===")
    println("GT[1] centroid: (${gtCentroids[1].second.first.toInt()}, ${gtCentroids[1].second.second.toInt()})")
    val gt1CenterX = gtCentroids[1].second.first
    val gt1CenterY = gtCentroids[1].second.second
    
    val closeToGT1 = quads.filter { quad ->
    ...
    println("\nGT areas:")
    println("  GT0 area: approx ${((gt0[1].x - gt0[0].x) * (gt0[3].y - gt0[0].y)).toInt()} pixels")
    println("  GT1 area: approx ${((gt1[1].x - gt1[0].x) * (gt1[3].y - gt1[0].y)).toInt()} pixels")
    println("  GT2 area: approx ${((gt2[1].x - gt2[0].x) * (gt2[3].y - gt2[0].y)).toInt()} pixels")
  }
