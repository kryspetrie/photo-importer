 ...
 *
 * Uses a real [YoloFaceDetectionService] instance created from the detection model on the
 * classpath. Only [parseFaceOutput] is tested — not inference — so the model choice doesn't matter;
 * we only need a valid ONNX session to construct the service.
 *
...
class YoloFaceDetectionServiceTest {
    ...
    inner class EmptyAndEdgeCases {
        ...
        fun returnsEmptyWhenBelowThreshold() {
            ...
            val output = arrayOf(arrayOf(floatArrayOf(100f, 80f, 200f, 180f, 0.3f, 0f)))
            val result = parseFaceOutput(output, 1f, 0, 0, 640, 480, 0.5f, 0.45f)
            assertThat(result).isEmpty()
        }
        ...
        fun filtersZeroAreaDetections() {
            val output = arrayOf(arrayOf(floatArrayOf(100f, 80f, 100f, 80f, 0.9f, 0f)))
            val result = parseFaceOutput(output, 1f, 0, 0, 640, 480, 0.5f, 0.45f)
            assertThat(result).isEmpty()
        }
    }
    ...
    inner class NmsFormat {
        ...
        fun parsesSingleDetection() {
            val output = arrayOf(arrayOf(floatArrayOf(270f, 180f, 370f, 300f, 0.9f, 0f)))
            val result = parseFaceOutput(output, 1f, 0, 0, 640, 480, 0.5f, 0.45f)
