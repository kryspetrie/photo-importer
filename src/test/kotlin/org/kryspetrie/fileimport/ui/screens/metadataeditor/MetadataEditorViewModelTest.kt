...
class MetadataEditorViewModelTest {
    ...
    inner class NearestCorrectionDegTests {
        @Test fun `returns 0 for NONE rotation`() {
            val result = OrientationCorrectionService.CorrectionResult(
                orientationDegrees = 0f, confidence = 0.95f,
                nearestRotation = RotationAngle.NONE, correctionDegrees = 0f, isJpeg = false,
            )
            // Test via static computation — the method is pure logic
            assertThat(when (result.nearestRotation) {
        ...
        @Test fun `returns 90 for CW_90 rotation`() {
            val result = OrientationCorrectionService.CorrectionResult(
                orientationDegrees = 270f, confidence = 0.95f,
                nearestRotation = RotationAngle.CW_90, correctionDegrees = 90f, isJpeg = false,
            )
            assertThat(when (result.nearestRotation) {
                RotationAngle.NONE -> 0
                RotationAngle.CW_90 -> 90
        ...
        @Test fun `returns 180 for CW_180 rotation`() {
            val result = OrientationCorrectionService.CorrectionResult(
                orientationDegrees = 180f, confidence = 0.95f,
                nearestRotation = RotationAngle.CW_180, correctionDegrees = 180f, isJpeg = false,
            )
            assertThat(when (result.nearestRotation) {
                RotationAngle.NONE -> 0
                RotationAngle.CW_90 -> 90
        ...
        @Test fun `returns 270 for CCW_90 rotation`() {
            val result = OrientationCorrectionService.CorrectionResult(
                orientationDegrees = 90f, confidence = 0.95f,
                nearestRotation = RotationAngle.CCW_90, correctionDegrees = 270f, isJpeg = false,
            )
            assertThat(when (result.nearestRotation) {
                RotationAngle.NONE -> 0
                RotationAngle.CW_90 -> 90
    ...
    inner class BulkEditStateTests {
        @Test fun `loadFiles initializes state correctly`() {
        ...
        @Test fun `loadSingleFile sets sourcePath and single file`() {
            val state = BulkEditState()
        ...
        @Test fun `updateSelectedConfig marks file as modified`() {
            val state = BulkEditState()
        ...
        @Test fun `updateConfig updates specific index`() {
            val state = BulkEditState()
            state.loadFiles(listOf(File("/tmp/a.jpg"), File("/tmp/b.jpg")))
        ...
        @Test fun `markSaved clears modified flag`() {
            val state = BulkEditState()
        ...
        @Test fun `showError and showInfo set messages`() {
            val state = BulkEditState()
        ...
        @Test fun `nextFile and prevFile navigate correctly`() {
            val state = BulkEditState()
        ...
        @Test fun `hasMetadata returns true when description is set`() {
            val config = org.kryspetrie.fileimport.domain.model.PhotoScanConfiguration(description = "test")
        ...
        @Test fun `cycleRotationCW rotates 90 degrees`() {
        ...
        @Test fun `cycleRotationCCW rotates 270 degrees`() {
            val config = org.kryspetrie.fileimport.domain.model.PhotoScanConfiguration()
            val rotated = config.cycleRotationCCW()
            assertThat(rotated.rotationDegrees).isEqualTo(270)
        }

        @Test fun `rotate180 rotates 180 degrees`() {
            val config = org.kryspetrie.fileimport.domain.model.PhotoScanConfiguration()
        ...
        @Test fun `modifiedCount counts modified files`() {
            val state = BulkEditState()
            state.loadFiles(listOf(File("/tmp/a.jpg"), File("/tmp/b.jpg")))
            assertThat(state.modifiedCount).isEqualTo(0)
